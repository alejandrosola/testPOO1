...
public class PruebaFiguras extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		elipseGirada = new Elipse(2, 1, new Punto(3, 3), Math.PI / 2, Color.red, Color.green);
		// elipse con radios 3 y 2, sin girar y con centro en 0;0
		elipseCentrada = new Elipse(3, 2, Color.red, Color.green);
		// c�rculo con centro en 0;0:
		circulo = new Elipse(2, 2, Color.red, Color.green);
		// tri�ngulo rect�ngulo:
		Punto[] t = { new Punto(0, 0), new Punto(0, 3), new Punto(4, 0) };
		triangulo = new Poligono(t, Color.red, Color.black);
		// cuadrado:
		Punto[] c = { new Punto(0, 0), new Punto(0, 1), new Punto(1, 1), new Punto(1, 0) };
		cuadrado = new Poligono(c, Color.red, Color.orange);
		// rect�ngulo:
		Punto[] r = { new Punto(0, 0), new Punto(0, 3), new Punto(5, 3), new Punto(5, 0) };
		rectangulo = new Poligono(r, Color.red, Color.pink);
		// figura compuesta:
	...
	public void testTipo() {
		...
		assertEquals("c�rculo", circulo.tipo());
		assertEquals("tri�ngulo", triangulo.tipo());
		assertEquals("cuadrado", cuadrado.tipo());
		assertEquals("cuadril�tero", rectangulo.tipo());
